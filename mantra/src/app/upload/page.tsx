...
import {
  ...
  X, Plus, Tag, Check, BookOpen, ArrowLeft
} from "lucide-react";
...
const CONTENT_TYPES = [
  ...
  },
  {
    ...
    icon: Video,
    title: "Video Lecture",
    description: "High-quality video teaching content",
  ...
  },
  {
    ...
    icon: Radio,
    title: "Live Session",
    description: "Interactive real-time mentoring or tutoring",
...
export default function UploadPage() {
  ...
  const [form, setForm] = useState({
    ...
    university: "", semester: "", tags: [] as string[], pricing: "free", price: ""
  });
  ...
  const addModule = () => setModules(prev => [...prev, { title: "", type: "lecture" }]);
      ...
      <main className="flex-1 max-w-[800px] mx-auto px-4 md:px-6 py-10 w-full">
            ...
            <div key={s} className="flex items-center gap-2 shrink-0">
              ...
              >
                <span className={cn(
                  ...
                  step > i ? "bg-secondary text-on-secondary" : step === i ? "bg-on-secondary-container text-secondary-container" : "bg-outline-variant/30"
                )}>
        ...
        {step === 0 && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <h2 className="font-manrope font-semibold text-lg text-primary mb-6">
            ...
            {CONTENT_TYPES.map((type) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-manrope font-semibold text-base text-primary group-hover:text-secondary transition-colors">
                    {type.title}
                  </p>
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-primary mb-1.5">Repository title *</label>
              <input
                value={form.title}
            ...
            <div>
              <label className="block text-sm font-medium text-primary mb-1.5">Department / Category</label>
            ...
            {/* Cover upload */}
            <div>
              <label className="block text-sm font-medium text-primary mb-1.5">Cover thumbnail</label>
              <div
                ...
                className={cn(
                  ...
                  isDragging
                    ...
                    : "border-outline-variant/40 hover:border-secondary/40 hover:bg-surface-container"
                )}
              >
                <Upload className="w-8 h-8 text-on-surface-variant mx-auto mb-3" />
                <p className="text-sm text-on-surface-variant">
                  Drop your image here, or{" "}
                  <span className="text-secondary font-medium cursor-pointer hover:underline">browse</span>
                </p>
                <p className="text-xs text-on-surface-variant/50 mt-1">PNG, JPG up to 5MB</p>
              </div>
            </div>

            {/* Pricing */}
            <div>
              <label className="block text-sm font-medium text-primary mb-2">Access</label>
              <div className="flex gap-2">
                {["free", "paid"].map(p => (
                  <button
                    key={p}
                    onClick={() => updateForm("pricing", p)}
                    className={cn(
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="card p-6 space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-manrope font-semibold text-base text-primary">{form.title || "Untitled Repository"}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-outline-variant/10 text-sm">
                <div>
                  ...
                  <span className="font-medium text-primary">{form.courseCode || "—"}</span>
                </div>
                <div>
                  <span className="text-on-surface-variant">Semester:</span>{" "}
                  <span className="font-medium text-primary">{form.semester || "—"}</span>
                </div>
                <div>
                  <span className="text-on-surface-variant">Modules:</span>{" "}
                  <span className="font-medium text-primary">{modules.filter(m => m.title).length}</span>
                </div>
                <div>
                  <span className="text-on-surface-variant">Access:</span>{" "}
                  <span className="font-medium text-primary capitalize">{form.pricing}</span>
                </div>
              </div>
              {form.tags.length > 0 && (
